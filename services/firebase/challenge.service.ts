...
import { Challenge, ChallengeCategory, ChallengeDifficulty, ChallengeStatus, PrizeModel } from '../../types/challenges'
    ...
    static async createChallenge(
        ...
        try {
            ...
            if (stakeAmount < Config.MIN_CHALLENGE_STAKE) {
                throw new Error(`Minimum stake is ${Config.MIN_CHALLENGE_STAKE} Dcoins`)
            }

            //Checking if the user has the enough balance
            const hasBalance = await TokenService.hasEnoughBalance(creatorId, stakeAmount)
            if (!hasBalance) {
            ...
            const challengeData = {
                ...
                status: aianalysis.approved ? 'active' : 'rejected',
            ...
            await updateDoc(doc(db, Collections.USERS, creatorId), {
                chlangesCreated: (await this.getUserChallengeCount(creatorId)) + 1
            })
   ...
   */
    ...
    private static formatChallenge(id: string, data: any): Challenge {
        return {
            ...
            aiAnalysis: {
                ...data.aiAnalysis,
                analysisTimestamp: data.aiAnalysis?.analysisTimestamp?.toDate() || new Date(),
            },
        } as Challenge;
    }
   ...
   */
    static async cancelChallenge(
        ...
        try {
            ...
            if (challenge.status !== 'active' && challenge.status !== 'pending_moderation') {
